struct UniformSampler
    ...
    UniformSampler()::UniformSampler = new(ChaCha12Stream())
end
...
"""
uniform_binary(us::UniformSampler, N::Int64, hw::Int64=0)::Vector{Int64} = begin
    if hw == 0
        rand(us.rng, [0, 1], N)
    else
        res = zeros(Int64, N)
...
"""
uniform_ternary(us::UniformSampler, N::Int64, hw::Int64=0)::Vector{Int64} = begin
    if hw < 0 || hw > N
        throw(DomainError("The Hamming weight must be between 0 and N."))
    end
...
"""
uniform_random(us::UniformSampler, N::Int64, Q::Modulus)::Vector{UInt64} = rand(us.rng, 0:Q.Q-1, N)

uniform_random(us::UniformSampler, N::Int64, Q::Moduli)::Vector{Vector{UInt64}} = begin
...
uniform_random(us::UniformSampler, Q::Modulus)::UInt64 = rand(us.rng, 0:Q.Q-1)

uniform_random_to!(us::UniformSampler, x::AbstractVector{UInt64}, Q::Modulus)::Nothing = begin
    N = length(x)
...
uniform_random_to!(us::UniformSampler, x::AbstractVector{Vector{UInt64}}, Q::Moduli)::Nothing = begin
    if length(x) ≠ length(Q)
        throw(DimensionMismatch("The array size does not match the moduli."))
    end
    @inbounds for i = eachindex(Q)
        N = length(x[i])
        for j = 0:8:N-8
...
samplebit(rng::ChaChaStream)::UInt8 = rand(rng, UInt8) & one(UInt64)

const tailcut::Int64 = 6
...
struct CDTSampler
    ...
    function CDTSampler(center::Real, sigma::Real)::CDTSampler
...
sample(s::CDTSampler)::Int64 = searchsortedlast(s.table, rand(s.rng, UInt64)) + s.cint + s.taillow

const baselog::Int64 = 10
const sampledepth::Int64 = 3
const hipreclog::Int64 = baselog * sampledepth
const lowpreclog::Int64 = 53 - hipreclog

struct VarCenSampler
    basesamplers::Vector{CDTSampler}

    function VarCenSampler(sigma::Real)::VarCenSampler
        bk = 0.0
...
function sample(s::VarCenSampler, center::Float64)::Int64
    cInt = floor(Int64, center)
    cFrac = center - cInt
...
function sampleC(s::VarCenSampler, c::Int64)::Int64
    mask = (1 << baselog) - 1
...
struct TwinCDTSampler
    ...
    function TwinCDTSampler(σ::Real)::TwinCDTSampler
...
function compute_cdt(c::Real, σ::Real)::Vector{UInt64}
    tail_hi = ceil(Int64, tailcut * σ)
    tail_lo = -tail_hi
...
function sample(centre::Float64, s::TwinCDTSampler)::Int64
    cfloor = floor(centre)
...
struct RGSampler
    ...
    function RGSampler()::RGSampler
        new(ChaCha12Stream(), 3.2)
    end

    function RGSampler(τ::Real)::RGSampler
        new(ChaCha12Stream(), τ)
    end
end

sample(s::RGSampler)::Int64 = round(Int64, randn(s.rng) * s.τ / √(2π))

sample(s::RGSampler, τ::Real)::Int64 = round(Int64, randn(s.rng) * τ / √(2π))

sample128(s::RGSampler, τ::Real)::Int128 = round(Int128, randn(s.rng) * τ / √(2π))