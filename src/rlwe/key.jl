...
"""
binary_ringkey(us::UniformSampler, N::Int64, hw::Int64=0)::RLWEkey = RLWEkey(uniform_binary(us, N, hw), N)
...
Otherwise, it outputs a secret key with hamming weight at most hw. 
"""
ternary_ringkey(us::UniformSampler, N::Int64, hw::Int64=0)::RLWEkey = RLWEkey(uniform_ternary(us, N, hw), N)
...
RLWEkeyPQ(key::RLWEkey, eval::PolyEvaluatorRNS)::RLWEkeyPQ = begin
    coeffs = [Vector{UInt64}(undef, key.N) for _ = eachindex(eval)]
    @inbounds for i = eachindex(eval)
        for j = 1:key.N
            coeffs[i][j] = Bred(key.coeffs[j], eval[i].Q)
        end
    end
