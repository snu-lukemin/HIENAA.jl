function get_c2s_matrix(oper::BFVOperator; packlen::Int64=0)::PlainMatrix
    ...
    if oper.packer.k % packlen ≠ 0
        throw(ErrorException("The number of slots must divide the packing parameter."))
    end
...
function get_s2c_matrix(oper::BFVOperator; packlen::Int64=0)::PlainMatrix
    if ismissing(oper.packer)
        throw(ErrorException("The operator must have a packer."))
    end
    if !isa(oper.packer, IntPackerSubring)
